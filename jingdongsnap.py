...
def parseArgs():
    ...
    parser.add_argument('--interval', dest='interval', help='抢购商品时查询商品是否可以购买的时间间隔(单位秒)', type=float, required=False, default=1800)
...
class JingDongSnap():
    ...
    def run(self):
        ...
        good_ids = good_id.split(',')

        for gid in good_ids:
            assert gid in cart_infos, '输入的商品编号有误'

        # 根据选择尝试购买商品
        for gid in good_ids:
            t = threading.Thread(target=self.run_thread, args=(cart_infos, gid))
            t.start()
    ...
    def run_thread(self, cart_infos, good_id):
    ...
    def buycartinfo(self):
        ...
        for _, item in enumerate(response_json['resultData']['cartInfo']['vendors']):
            for iitem in item['sorted']:
                if 'skuUuid' in iitem['item']:
                    cart_info = self.getCartInfo(iitem)
                    cart_infos[iitem['item']['Id']] = cart_info
                else:
                    for iiitem in iitem['item']['items']:
                        cart_info = self.getCartInfo(iiitem)
                        cart_infos[iiitem['item']['Id']] = cart_info

        return cart_infos

    def getCartInfo(self, item):
        return {
            'title': item['item']['Name'],
            'Id': item['item']['Id'],
            'skuUuid': item['item']['skuUuid'],
            'IdForOldVersion': item['item']['IdForOldVersion'],
            'SType': '11',
        }
